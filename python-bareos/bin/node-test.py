...
def getArguments():
    ...
    argparser.add_argument(
        "path", action="store", help="path", nargs='?', default=""
    )    
...
if __name__ == "__main__":
    ...
    bareos = Root(director, None, None, None)
    
    try:
        children = bareos.readdir(Path(args.path), None)
        pprint(children)
    except AttributeError:
        children = None
    try:
        content = bareos.read(Path(args.path), None, None)

...
class BvfsCommon(Base):
    ...
    def init(self, root, jobid, path, filename, stat):
        self.jobid = jobid
        self.static = True
        self.restorepath = os.path.normpath(
            "/%s/jobid=%s/" % (self.root.restorepath, self.jobid)
        )
        if filename:
            self.restorepathfull = os.path.normpath(
                "%s%s%s" % (self.restorepath, path, filename)
            )
        else:
    ...
    def restore(self, path, pathIds, fileIds):
        ...
        if self.root.restorejob:
            restorejob = "restorejob={restorejob} ".format(
                restorejob=self.root.restorejob
            )
        ...
        try:
            ...
            self.logger.debug(
                "failed to get resulting jobid of run command (maybe old version of Bareos Director)"
            )
        cleanup = self.bsock.call(
            ".bvfs_cleanup path={bvfs_restore_id}".format(
                bvfs_restore_id=bvfs_restore_id
            )
        )

        ...
        if path.len() == 0:
            ...
            if key == self.XattrKeyRestoreTrigger and value == b"restore":
                if not self.root.restoreclient:
                    self.logger.warning(
                        "no restoreclient given, files can not be restored"
                    )
                    return -errno.ENOENT
                if not self.root.restorepath:
                    self.logger.warning(
                        "no restorepath given, files can not be restored"
                    )
                    return -errno.ENOENT
                else:
            ...
            for i in directories:
                if i["name"] != "." and i["name"] != "..":
                    if i["name"] == "/":
                        ...
                        BvfsCommon.init(
                            self, self.root, self.jobid, i["fullpath"], None, i["stat"]
                        )
                    else:
                        found = True
            if not found:
